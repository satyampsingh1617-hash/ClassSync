<template>
  <AppLayout>
    <div class="mb-6">
      <h2 class="text-lg font-semibold text-gray-900">My Attendance History</h2>
      <p class="text-sm text-gray-500">Detailed attendance records by subject</p>
    </div>
      ...
      <div class="flex gap-2 flex-wrap mb-5">
        <button
          ...
          :class="[
            ...
            !activeSubject ? 'bg-brand-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
          ]"
        >All Subjects</button>
        <button
          ...
          :class="[
            'px-4 py-2 rounded-lg text-sm font-medium transition-all',
            activeSubject === sub.subject?._id ? 'bg-brand-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
          ]"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
        ...
        >
          <div class="flex items-start justify-between mb-3">
            <div>
              <h4 class="font-semibold text-gray-900">{{ sub.subject?.name }}</h4>
              <span class="font-mono text-xs bg-orange-50 text-orange-700 px-2 py-0.5 rounded mt-1 inline-block">{{ sub.subject?.code }}</span>
            </div>
            <span :class="[
              'text-lg font-bold',
              parseFloat(sub.percentage) >= 75 ? 'text-green-600' : 'text-red-500'
            ]">{{ sub.percentage }}%</span>
          </div>
          ...
          <div class="w-full bg-gray-100 rounded-full h-2 mb-2">
            <div
              ...
              :class="parseFloat(sub.percentage) >= 75 ? 'bg-green-500' : 'bg-red-400'"
              :style="{ width: `${Math.min(100, sub.percentage)}%` }"
            ></div>
          </div>

          <div class="flex justify-between text-xs text-gray-500">
            <span>Present: <strong class="text-green-600">{{ sub.present }}</strong></span>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(rec, i) in filteredRecords" :key="rec._id" class="hover:bg-gray-50">
                <td class="table-td text-gray-400">{{ i + 1 }}</td>
                <td class="table-td">
                  <div>
                    <p class="font-medium text-gray-900">{{ rec.subjectId?.name }}</p>
                    <p class="text-xs text-gray-400">{{ rec.subjectId?.code }}</p>
                  </div>
                </td>
              ...
              <tr v-if="!filteredRecords.length">
                <td colspan="5" class="table-td text-center text-gray-400 py-10">No records found</td>
              </tr>
            </tbody>
          </table>
        </div>
