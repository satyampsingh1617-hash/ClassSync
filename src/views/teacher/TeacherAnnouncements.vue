  ...
  <AppLayout>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-surface-500">Post updates for your students</p>
      </div>
    ...
    <div v-else class="space-y-4">
      ...
      <div v-for="ann in announcements" :key="ann._id" class="card hover:shadow-card-md transition-shadow">
        <div class="flex items-start justify-between gap-4">
          ...
          <button
            v-if="ann.createdBy === myUserId"
            @click="confirmDelete(ann)"
            class="flex-shrink-0 p-1.5 rounded-lg text-surface-400 hover:text-danger hover:bg-danger-light transition-all"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <ModalDialog :show="showDelete" title="Delete Announcement" @close="showDelete=false">
      <p class="text-sm text-surface-600 mb-6">Delete "<strong>{{ deleteTarget?.title }}</strong>"?</p>
